    ...
    if (e.target.value.includes("SCREEN_SCAN")) {
      for (item of items) {
        if (barcodeCheck.includes(item.innerHTML.toUpperCase())) {
          ...
          brcd.src = `https://www.webarcode.com/barcode/image.php?code=${item.innerHTML}&type=C128B&xres=1&height=50&width=250&font=3&output=png&style=68`;
          brcd.width = "200";
      ...
      if (e.target.value != item.innerHTML.toUpperCase()) {
        ...
        if (mappedItems) {
          for (item of mappedItems) {
            if (
              e.target.value &&
              e.target.value.toUpperCase() == item.oldSku.toUpperCase()
            ) {
